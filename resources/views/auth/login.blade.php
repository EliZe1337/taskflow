    ...
    <div class="bg-white rounded-2xl shadow-2xl p-8">
        ...
        <div class="mt-6 pt-5 border-t border-slate-100">
            ...
            <div class="grid grid-cols-2 gap-2">
                ...
                ] as [$role, $email, $classes])
                <button onclick="fillLogin('{{ $email }}')"
                    class="text-xs {{ $classes }} px-3 py-2 rounded-lg font-medium text-left hover:opacity-80 transition">
                    {{ $role }}<br>
                    <span class="font-normal opacity-75 text-xs">{{ $email }}</span>
                </button>
                @endforeach
            </div>
        </div>
