...
@section('header', 'Мои заметки')

    @section('content')
        <div class="flex items-center justify-between mb-6">
            <div>
                <p class="text-sm text-slate-500">Всего заметок: {{ $notes->count() }}</p>
            </div>
            <a href="/notes/create"
            ...
            @forelse ($notes as $note)
                <a href="/notes/{{ $note->id }}"
                   class="group bg-white rounded-xl border border-slate-200 p-5 hover:shadow-md hover:border-slate-300 transition-all">
                    ...
                    <h3 class="text-sm font-semibold text-slate-800 group-hover:text-blue-600 transition-colors truncate">{{ $note->title }}</h3>
                    @if($note->body)
                        <p class="text-xs text-slate-400 mt-1 line-clamp-2">{{ Str::limit($note->body, 80) }}</p>
                    @endif
                    @if($note->updated_at)
                        <p class="text-xs text-slate-300 mt-3">{{ $note->updated_at->diffForHumans() }}</p>
                    @endif
                </a>
            @empty
